...
export const getFilesSync = (dir) =>{
	
	const fileList = []
	const exists = existDirSync(dir)
	if(!exists) return fileList
    ...
    files.forEach((file) => {
        ...
        if (fs.statSync(filePath).isDirectory()) {
            getFilesSync(filePath, fileList);
        } else {
...
export const existDirSync = (dirPath) =>{
...
export const writeFileSync = async ( path, data ) => fs.writeFileSync( path, data, 'utf8' )
...
export const readFileSync = projectPath => fs.readFileSync( projectPath )
export const readJSON = async projectPath => JSON.parse( readFileSync( projectPath ) )
export const readJSONSync = projectPath => JSON.parse( readFileSync( projectPath ) )
export const pkgFunct = fileName => {

	const json = projectPath => readJSON( projectPath )
