...
export const getData = async ( {
	...
	const getOpts = ( value: ClippoArgs['opts'], help?: ClippoHelp, fallbackHelp?: ClippoHelp )=> {
		...
		const opts = Object.entries( value ).map( ( [ k, v ] ) => {
			...
			const values = [ ...( v.alias ?? [] ), k ].map( value => {
	...
	const getHelpParams = ( {
		name, params, fallbackParams, isGlobal, 
	}:{name: string, params: Command, fallbackParams?: Command, isGlobal?: boolean} ) => {
	...
	const getChildsData = ( {
		name, params, fallbackKeys, fallbackParams,
	}: ChildData ) => {
